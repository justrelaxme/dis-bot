...
if (probe.encrypted) {
  say(
    probe.pooled
      ? 'Соединение шифруется. Идём через pooler: участок до Postgres внутренний, и он не шифруется — так у Supabase и устроено.'
      : 'Соединение шифруется.',
  );
} else {
  say('Соединение НЕ шифруется. Для облачной базы это повод проверить sslmode в строке подключения.');
}
say(verdictOf(probe));
