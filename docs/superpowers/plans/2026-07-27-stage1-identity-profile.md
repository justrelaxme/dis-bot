...
- **Сообщения zod задаются параметром схемы** (`z.string({ error: '…' })`), а не вторым аргументом `.min()` / `.regex()`: второй аргумент покрывает только свою проверку, и при отсутствующем значении наружу протекает английский текст zod. Актуально для валидации ответов провайдеров.
