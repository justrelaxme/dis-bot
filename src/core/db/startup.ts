...
/**
 * Что написать в лог. Одна строка, из которой понятно, чинить базу, тариф, пароль или код —
 * потому что чинят их разные люди и разными способами.
 */
export function explainStartupFailure(failure: StartupFailure): string {
  switch (failure) {
    case 'quota':
      return 'база отказала по тарифу: у неё кончилась квота (у Neon это вычислительное время). Код и миграции тут ни при чём — бот не стартует, пока квота не восстановится или не сменится тариф';
    case 'unreachable':
      return 'база не отвечает — бот не стартует. Проверьте DATABASE_URL, доступность хоста и то, что база поднята: внутри контейнера localhost — это сам контейнер, и базы там нет';
    case 'auth':
      return 'база не пустила: не подошли пароль, имя базы или прав недостаточно — бот не стартует';
    case 'migration':
      return 'миграции не применились — бот не стартует';
  }
}
