...
const envSchema = z.object({
  ...
  /**
   * Cookie аккаунта HoYoLAB — строкой целиком, как её отдаёт браузер (важны `ltoken_v2` и
   * `ltuid_v2`). Единственный способ прочитать полный состав аккаунта Genshin: публичного API
   * у HoYoverse нет, а внутренний без авторизации не отвечает.
   *
   * Ключ здесь **владельца бота, а не игроков**: cookie нужны только чтобы запрос приняли,
   * данные при этом отдаются про любой UID с публичной Летописью. Чужие ключи бот не
   * спрашивает.
   *
   * Без переменной состав остаётся заявленным игроком — как ранг Valorant. Ничего не ломается.
   */
  HOYOLAB_COOKIE: emptyToUndefined(z.string().min(1).optional()),

  /**
   * Применять миграции при старте. По умолчанию да: на платформах с одним контейнером
   * отдельного шага миграций нет, и без этого первый же деплой с новой колонкой уронил бы
   * бота. В docker-compose миграции делает отдельный сервис, и повторный прогон здесь —
   * пустая операция, потому что drizzle применяет только неприменённое.
   */
  MIGRATE_ON_START: z
  ...
  /**
   * Регистрировать slash-команды при старте. По умолчанию да, и по той же причине, что и
   * миграции: отдельный шаг для этого существовал, но требовал `tsx` — а он devDependency,
   * и установка без dev-зависимостей оставляла деплой без нового набора команд. Молча:
   * бот поднимался, работал, и только команды оставались прошлыми.
   *
   * Один PUT на запуск в лимиты не упирается (их двести в сутки на сервер), а набор команд
   * при этом всегда тот, что в коде. Выключать стоит там, где команды ставят отдельным
   * шагом и лишнее обращение к Discord при каждом перезапуске не нужно.
   */
  DEPLOY_COMMANDS_ON_START: z
    .enum(['true', 'false'], { error: 'допустимо: true или false' })
    .default('true')
    .transform((value) => value === 'true'),

  /**
   * Где лежат файлы миграций. По умолчанию относительно рабочего каталога — так удобно в
   * разработке. В образе задан абсолютный путь: рабочий каталог процесса выбирает
   * платформа, и относительный путь искал бы миграции там, куда она нас поставит.
   */
  MIGRATIONS_DIR: z.string().min(1).default('src/core/db/migrations'),

  /**
   * Отдавать ли `/metrics`. Выключено по умолчанию: раньше этот путь закрывал от внешнего
   * мира только Caddy, то есть защита жила в обвязке. На платформе, которая даёт домен
   * сама и Caddy не имеет, счётчики оказались бы публичными — имена команд, число ошибок,
   * имена провайдеров. Если endpoint не должен быть виден снаружи, это должен решать код:
   * обвязка может отсутствовать, а код едет вместе с приложением.
   */
  METRICS_ENABLED: z
    .enum(['true', 'false'], { error: 'допустимо: true или false' })
    .default('false')
