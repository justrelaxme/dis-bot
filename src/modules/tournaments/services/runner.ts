...
async function runGuild(
  ...
  if (cycle.stage === 'registration') {
    ...
    // Турнир уже стартовал — руками организатора или прошлым тиком, который упал после того,
    // как сетка записалась. Стадия дня догоняет турнир, а не пытается стартовать его заново:
    // повторный старт отказал бы, и день застрял бы в регистрации навсегда.
    const already = await deps.tournaments.byId(cycle.tournamentId);
