...
export const tournamentRosters = pgTable(
  'tournament_rosters',
  {
    ...
    characters: jsonb('characters')
      ...
      .notNull(),
    /**
     * Кого игрок защитил от бана. Подмножество заявленных: защищать того, кого не берёшь,
     * незачем, и такая заявка означала бы, что человек ошибся.
     */
    immune: jsonb('immune').$type<string[]>().notNull().default([]),
...
export const tournamentFormats = pgTable(
  'tournament_formats',
  {
    ...
    /**
     * Сколько персонажей игрок может защитить от бана — иммуны.
     *
     * Правило из турниров сообщества: каждый называет своих неприкасаемых, и соперник их
     * забанить не может. Ноль означает, что иммунов нет вовсе и банить можно кого угодно.
     *
     * Настройка организатора, а не игры: от числа иммунов зависит, во что превращается драфт.
     * Один иммун оставляет банам смысл, а восемь при восьми пиках отменяют их целиком.
     */
...
export const tournaments = pgTable(
  'tournaments',
  {
    ...
    costCap: doublePrecision('cost_cap'),
    /**
     * Сколько иммунов у каждого игрока в этом турнире. Снимок формата на момент запуска:
     * правило вечера не должно меняться от того, что формат поправили назавтра.
     */
    immunities: integer('immunities').notNull().default(0),
    /**
...
export const tournamentMatchReports = pgTable('tournament_match_reports', {
  ...
  action: text('action')
    .$type<'report' | 'confirm' | 'dispute' | 'resolve' | 'walkover' | 'auto-confirm' | 'verified' | 'replay' | 'correct'>()
    .notNull(),
      ...
      .$type<
        {
          ...
          owned?: ('a' | 'b')[];
          /** Кто заявил состав сам. Только по этой пометке драфт запрещает пик. */
          declaredBy?: ('a' | 'b')[];
          /** Для кого персонаж неприкасаем: забанить его нельзя. */
          immuneFor?: ('a' | 'b')[];
          /**
