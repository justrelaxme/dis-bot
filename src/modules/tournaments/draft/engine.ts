...
export function canChoose(
   ...
   * прочитан или заявлен: взять того, кого у тебя нет, значит выйти на этаж без него, и
   * разбираться пришлось бы уже после матча.
   *
   * Проверка стоит только на пике. Бан чужого — законный и осмысленный ход: банят как раз то,
   * что есть у соперника, а не у себя.
   *
   * Отсутствие пометки означает «неизвестно», а не «ни у кого»: без ключа HoYoLAB, при
   * закрытой Летописи и у остальных дисциплин пометок нет вовсе, и драфт идёт как раньше.
   */
  if (view.current.kind === 'pick' && option.owned && !option.owned.includes(side)) {
    return {
      ok: false,
