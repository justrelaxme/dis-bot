...
export interface DraftOption {
  ...
  owned?: DraftSide[];
  /**
   * Кто заявил состав сам, а не был просто прочитан из Летописи.
   *
   * Разница определяет, запрет это или подсказка. Заявку человек сделал своими руками, и
   * держать его в её пределах честно. А прочитанное из Летописи бот мог увидеть устаревшим —
   * вчерашняя крутка появляется там не сразу, — и запрещать по таким данным значило бы
   * отнимать ход из-за задержки чужого сервиса.
   */
  declaredBy?: DraftSide[];
  /**
   * Для кого этот персонаж неприкасаем. Иммун объявляет игрок в своей заявке, а сколько их
   * можно — задаёт организатор форматом.
   *
   * Забанить иммунного персонажа соперник не может: в этом всё правило. Взять его при этом
   * может кто угодно — пул зеркальный, и иммун защищает от бана, а не присваивает.
   */
  immuneFor?: DraftSide[];
  /**
