...
describe('createScheduler', () => {
  ...
  it('регистрирует выполняющуюся джобу в shutdown, чтобы drain её дождался', async () => {
    // croner.stop() не прерывает уже идущий прогон — единственная защита от того,
    // что SIGTERM оборвёт джобу на середине, это участие в drain через shutdown.track.
    let finished = false;
    const run = vi.fn(async () => {
    ...
    const runPromise = scheduler.runOnce('sync');
